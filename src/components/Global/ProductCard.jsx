...
function ProductCard({product}) {
    return ( 
        <Link to={`/book/${product.title.toLowerCase()}`} onClick={() => {localStorage.setItem('data', JSON.stringify(product))}} key={product.key} className="border-solid border border-black grow shrink-0">
